    ...
    public readonly struct SerialPortDeviceAddress : IDeviceAddress<string>, IEquatable<SerialPortDeviceAddress>
    {
        ...
        public SerialPortDeviceAddress(int deviceAddress)
        {
            ...
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
            {
                ...
                if (!File.Exists(Address)) Address = $"/dev/ttyACM{deviceAddress}";

                // Check if Linux ACM port exists, if not try to connect via AMA
                if (!File.Exists(Address)) Address = $"/dev/ttyAMA{deviceAddress}";

                // Check if Linux ACM port exists, if not try to connect via SAC
                if (!File.Exists(Address)) Address = $"/dev/ttySAC{deviceAddress}";

                // Check if Linux ACM port exists, if not try regular COM/RS232 port
                if (!File.Exists(Address)) Address = $"/dev/ttyS{deviceAddress}";
            }
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            {
                throw new PlatformNotSupportedException(
                    "OSX port number uses weird naming for USB and thus is not supported via numeric index");
            }
            else
                throw new PlatformNotSupportedException();
        }
        ...
        public static bool operator ==(SerialPortDeviceAddress left, SerialPortDeviceAddress right)
